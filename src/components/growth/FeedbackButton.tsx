...
// Growth OS — the continuous feedback loop, capture end. A small, warm control
...
import { StarIcon, CloseIcon, CheckIcon, RefreshIcon } from '@/components/icons'
...
const CHANGELOG = [
  { v: '2.4', when: 'This release', what: 'Growth OS fully built out — Content Engine, Lead Gen, Automations, the 18-agent AI Team, and the Triage Strip, all native to BDR Hub.' },
  { v: '2.3', when: 'Earlier', what: 'AI Coach now reasons from your real Growth OS state — goals, pipeline temperature, and which agents are live.' },
...
export function FeedbackButton() {
  ...
  const reset = () => { setOpen(false); setTimeout(() => { setSent(false); setSentiment(null); setDetail('') }, 200) }
    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-[1050] flex items-center justify-center bg-dark-text/50 p-4" onClick={reset}>
          <div className="w-full max-w-sm rounded-2xl bg-card p-5 shadow-modal" onClick={e => e.stopPropagation()}>
            <div className="mb-3 flex items-center justify-between">
              <h2 className="text-[16px] font-[900] text-dark-text">Help shape Growth OS</h2>
            ...
            {sent ? (
              ...
              <>
                <p className="mb-3 text-[12.5px] leading-relaxed text-gray">Is Growth OS steering you well? Your rating rolls into the daily improvement digest.</p>
