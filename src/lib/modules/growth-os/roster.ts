...
export interface RosterAgent {
  ...
  job: string                // what it does, plain language (compact summary)
  details: string[]          // bulleted "what it does + what it's looking for"
...
export const ROSTER: RosterAgent[] = [
  ...
  { id: 'scoring', name: 'Lead Scoring Agent', category: 'funnel', defaultStatus: 'live', costPerMo: 18,
    ...
    details: [
      'Reads the full form submission the instant a partner prospect is created — before any human sees it.',
      'Scores three dimensions — Intent (0–40), Fit (0–35), Readiness (0–25) — and sums them to a 0–100 total.',
      'Looks for: an established agency with an active client base, a stated need for a credit product, and how soon they can move.',
      'Writes the score, a one-sentence rationale, and a stage (hot / warm / cold) back onto the CRM record.',
      'Routes the lead onward — hot to the Speed-to-Lead Alert, warm to Nurture Drip, cold to Cold Nurture — never leaving one unrouted.',
    ],
...
Never fabricate a fact to justify a score. Never let total exceed the sum of the sub-scores. Never skip the handoff — an unscored or unrouted lead is the worst failure mode in this system.` },
  ...
  { id: 'alert', name: 'Speed-to-Lead Alert Agent', category: 'funnel', defaultStatus: 'live', costPerMo: 12,
    ...
    details: [
      'Fires within ~60 seconds of a lead scoring 80+, before the qualification window closes.',
      'Extracts the single sharpest fact from the submission to open the first call with — one fact, not a summary.',
      'Sends an SMS to the BDR and posts to Slack with a link to the record, so the alert lands even if one channel fails.',
      'Looks for: a duplicate alert already sent, and whether a human has made contact yet.',
      'Starts a 5-minute escalation — re-pings Slack (never a second SMS) if the lead is still untouched.',
    ],
...
Never delay sending to "improve" the message — a good-enough alert in 10 seconds beats a perfect one in 2 minutes. Never alert twice for the same scoring event.` },
  ...
  { id: 'call-prep', name: 'Call Prep Agent', category: 'funnel', defaultStatus: 'setup', costPerMo: 9,
    ...
    details: [
      'Triggers the instant a partner call is booked and assembles a one-screen brief.',
      'Pulls the agency snapshot, their stated need, and any competitor intel from the Battle Cards library.',
      'Writes the single sharpest opening line — grounded in their situation, not a generic greeting.',
      'Looks for: the two objections most likely for this specific agency, each with a one-line rebuttal.',
      'Flags one thing NOT to do on the call, and delivers the whole brief 30 minutes beforehand.',
    ],
...
Never invent facts about the agency not present in the LEO. Never promise a client outcome — this is partner enablement, and CROA applies to anything they'd say downstream too.` },
  ...
  { id: 'noshow', name: 'No-Show Rescue Agent', category: 'funnel', defaultStatus: 'live', costPerMo: 7,
    ...
    details: [
      'Fires when an agency contact misses a booked call — the warmest recovery moment there is, with a short shelf life.',
      'Queries the calendar for the next three real open slots and presents them in plain, readable form (never invents one).',
      'Writes a short, warm, zero-guilt text with those slots and a booking link.',
      'Looks for: a no-show that was already handled, so it never double-texts.',
      'Escalates to a human if no slots exist in the next five business days.',
    ],
...
Never use guilt language ("we missed you"). Never invent calendar slots.` },
  ...
  { id: 'nurture', name: 'Nurture Drip Agent', category: 'funnel', defaultStatus: 'live', costPerMo: 22,
    ...
    details: [
      'Runs the 5-email warm sequence (scores 50–79) as one continuous conversation, not five interchangeable templates.',
      'Reads every prior email in the thread before writing the next — no reused opener, stat, or story.',
      'Escalates pressure gradually: rapport + one lesson → proof + objection → soft next step → clearest ask.',
      'Looks for: a reply — the instant one is logged it stands down and flags for human follow-up.',
      'Never sends two emails the same day; ends cleanly at step 5 with no sixth send.',
    ],
...
Never send two emails the same calendar day. Never continue after a reply is logged — a human response always beats automation.` },
  ...
  { id: 'cold-nurture', name: 'Cold Nurture Agent', category: 'funnel', defaultStatus: 'paused', costPerMo: 14,
    ...
    details: [
      'Runs a 6-email, 14-day pure-education drip for low-score agencies who aren\'t ready yet.',
      'Teaches one genuinely useful thing per email about running a credit-repair / broker practice — zero pitch until the final send.',
      'Looks for: engagement signals (2+ link clicks) that mark a lead as warming up.',
      'Promotes a qualifying lead straight into the warm Nurture Drip sequence at step 1.',
      'Never repeats a lesson already taught; never keeps an engaged, promoted lead in the cold track.',
    ],
...
Never pitch before the final email. Never keep an engaged, promoted lead in the cold track.` },
  ...
  { id: 'affiliate', name: 'Affiliate Attribution Agent', category: 'funnel', defaultStatus: 'setup', costPerMo: 11,
    ...
    details: [
      'Fires on a partner referral-link click and tags the attribution on the referred agency\'s record.',
      'Records the referring partner and the attribution window — accuracy matters because real payouts depend on it.',
      'Looks for: an existing earlier attribution inside the window, which it never overwrites.',
      'Selects the co-branded landing experience matching the referring partner.',
      'Notifies the partner on conversion — runs in shadow mode for a week first, since payouts are downstream.',
    ],
...
Never re-attribute a lead already attributed within its active window. Never notify a partner of a conversion the compliance/finance check hasn't confirmed.` },
  ...
  { id: 'winback', name: 'Win-Back Agent', category: 'funnel', defaultStatus: 'setup', costPerMo: 8,
    ...
    details: [
      'Scans daily for agencies gone quiet 30+ days with no active sequence.',
      'Brings a genuinely new reason to talk — never the angle that already went quiet.',
      'Looks for: a fresh hook worth a reply — a new Co-Brand PLUS+ capability, a proof point, a shift in their market.',
      'Keeps it short, specific, and easy to reply to; caps the effort at 21 days with no engagement.',
      'Promotes any re-engaged contact back into the warm Nurture Drip flow.',
    ],
...
Never reuse the angle that already failed. Never keep messaging past the 21-day cap.` },
  ...
  { id: 'content-idea', name: 'Content Idea Agent', category: 'content', defaultStatus: 'setup', costPerMo: 10,
    ...
    details: [
      'Drafts and ranks 6–8 content ideas every morning, each scored by expected value.',
      'Looks for: which formats, hooks, and pillars are currently earning the most engagement.',
      'Rebalances toward any under-served pillar — education, proof, engagement, social proof.',
      'Frames every idea for an AGENCY-owner audience (why a credit-repair business owner stops scrolling), never end-consumers.',
      'Anchors each EV estimate to real comparable past posts — never invented numbers.',
    ],
...
Never invent performance numbers — anchor EV to real comparable posts. Never frame content at end-consumers; the audience is the partner.` },
  ...
  { id: 'hook-research', name: 'Hook Research Agent', category: 'content', defaultStatus: 'setup', costPerMo: 8,
    ...
    details: [
      'Studies weekly which opening lines actually hold an agency owner\'s attention.',
      'Ranks hook archetypes by a blend of retention and expected value.',
      'Looks for: the top 2–3 patterns worth drafting fresh, audience-specific variants of.',
      'Marks each variant Proven vs Experiment honestly — never oversells an untested one.',
      'Feeds the validated hooks into the Content Idea Agent\'s queue.',
    ],
...
Never present an untested variant as Proven. Never write consumer-targeted hooks.` },
  ...
  { id: 'referral', name: 'Referral Ask Agent', category: 'retention', defaultStatus: 'live', costPerMo: 6,
    ...
    details: [
      'Fires right after a new partner experiences a real early win — the moment they\'re happiest with the decision.',
      'Looks for: a genuine positive-outcome event (first client activated, first co-branded enrollment) before it ever asks.',
      'Writes a warm ask that references the specific win, not a generic "hope you\'re happy".',
      'Keeps the ask low-friction — one clear next step, one name is enough.',
      'Routes through the Compliance Guardian before anything sends.',
    ],
...
Never ask before a real positive outcome exists. Never imply a guaranteed client result in the ask.` },
  ...
  { id: 'customer-retention', name: 'Partner Retention & Activation Agent', category: 'retention', defaultStatus: 'setup', costPerMo: 15,
    ...
    details: [
      'Watches signed partners\' activation and usage every day — recurring commission makes a saved partner worth more than a new lead.',
      'Ranks signed partners by churn risk from inactivity and stalled activation.',
      'Looks for: the specific blocker on each at-risk partner — stalled onboarding, no first client, gone quiet.',
      'Drafts the re-activation nudge that addresses that exact blocker, not a generic check-in.',
      'Routes partner-facing sends through the Compliance Guardian; never nudges a healthy, active partner as if at risk.',
    ],
...
Never promise the partner a client outcome. Never nudge a healthy, active partner as if they were at risk.` },
  ...
  { id: 'testimonial', name: 'Testimonial / UGC Agent', category: 'retention', defaultStatus: 'setup', costPerMo: 7,
    ...
    details: [
      'Fires when a partner hits a real milestone worth celebrating.',
      'Looks for: the specific milestone to reference, so the ask feels earned rather than generic.',
      'Writes a 30-second ask — one or two questions, answered in the partner\'s own words.',
      'Routes returned quotes into the Content Idea queue as proof — the highest-converting content there is.',
      'Gated through Compliance; never publishes a testimonial implying a guaranteed credit outcome.',
    ],
...
Never publish a testimonial implying a guaranteed credit outcome. Never fabricate a quote.` },
  ...
  { id: 'compliance-guardian', name: 'Compliance Guardian Agent', category: 'ops', defaultStatus: 'setup', costPerMo: 20,
    ...
    details: [
      'A synchronous gate, not a scheduled agent — every partner-facing message passes through it before it sends.',
      'Checks every claim against CROA, TCPA, and CAN-SPAM in one pass.',
      'Looks for: guaranteed credit-outcome language, missing SMS opt-out, missing email unsubscribe, and any misstated Co-Brand PLUS+ term.',
      'Returns approved — or rejected with the exact offending phrase, the rule it breaks, and a compliant rewrite.',
      'When uncertain, it rejects: a safe rewrite always beats a risky send, and a rejection is a normal part of the loop.',
    ],
...
Never approve a guaranteed-outcome claim, however softened. Never let a first-in-sequence SMS go without opt-out language. When uncertain, reject.` },
  ...
  { id: 'qa', name: 'Pipeline QA Agent', category: 'ops', defaultStatus: 'setup', costPerMo: 9,
    ...
    details: [
      'Audits the automation log across the whole pipeline nightly.',
      'Looks for: silent failures — a lead scored but never routed, an alert never sent, a sequence that kept sending after a reply, a duplicate send.',
      'Checks each contact\'s log tells a complete, valid story end to end (scored → routed → the right downstream agent fired).',
      'Ranks every flag by revenue impact so the costly ones surface first.',
      'Writes flags to a human review queue — never auto-modifies a record to "fix" an issue itself.',
    ],
...
Never auto-modify a contact to "fix" an issue — surface it for a human. Never bury a high-severity flag under low ones.` },
  ...
  { id: 'reconciliation', name: 'Income Reconciliation Agent', category: 'ops', defaultStatus: 'setup', costPerMo: 8,
    ...
    details: [
      'Compares expected recurring commission (from signed, active partners) against what actually landed, weekly.',
      'Looks for: meaningful variance — a churned partner, a delayed payout, a new signature.',
      'Attributes a variance to a cause only where the data supports it — never invents one.',
      'Stays quiet on routine, in-line results so it never manufactures noise.',
      'Feeds any notable variance to the Daily Briefing Agent.',
    ],
...
Never invent a cause the data doesn't support. Never flag routine, in-line results as problems.` },
  ...
  { id: 'daily-briefing', name: 'Daily Briefing Agent', category: 'ops', defaultStatus: 'setup', costPerMo: 6,
    ...
    details: [
      'Pushes the one or two most important things across the whole system to the BDR\'s phone each morning.',
      'Looks for: a hot lead needing contact — always the first line, no exceptions.',
      'Pulls from the same triage computation as the in-app strip, plus the hot-lead count and any recent reconciliation result.',
      'Synthesizes it all into a single message readable in 10 seconds on a phone.',
      'Sends an honest "nothing urgent today" on calm days rather than crying wolf.',
    ],
...
Never bury the most urgent item. Never exceed a 10-second read — that's what the in-app strip is for.` },
  ...
  { id: 'teaching', name: 'Teaching Agent', category: 'ops', defaultStatus: 'live', costPerMo: 0,
    ...
    details: [
      'The in-app guided walkthrough — already live and running client-side, nothing to deploy.',
      'Shows a brand-new BDR exactly where everything lives on their first visit.',
      'Looks for: the fastest path to their first real action, then gets out of the way.',
      'Replayable any time from the header, so it doubles as an on-demand refresher.',
      'Hands off to the AI Coach for the first real question a BDR wants answered.',
    ],
