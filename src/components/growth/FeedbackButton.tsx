...
const CHANGELOG = [
  { v: '2.4', when: 'This release', what: 'Agentic CRM fully built out — Content Engine, Lead Gen, Automations, the 18-agent AI Team, and the Triage Strip, all native to BDR Hub.' },
  { v: '2.3', when: 'Earlier', what: 'AI Coach now reasons from your real Agentic CRM state — goals, pipeline temperature, and which agents are live.' },
        ...
        <div className="fixed inset-0 z-[1050] flex items-center justify-center bg-dark-text/50 p-4" onClick={reset}>
          <div className={cn('w-full rounded-2xl bg-card p-5 shadow-modal', mode === 'digest' ? 'max-h-[85vh] max-w-md overflow-y-auto' : 'max-w-sm')} onClick={e => e.stopPropagation()}>
            <div className="mb-3 flex items-center justify-between">
              <h2 className="text-[16px] font-[900] text-dark-text">{mode === 'digest' ? 'Team feedback digest' : 'Help shape Agentic CRM'}</h2>
            ...
            {mode === 'digest' ? (
              ...
              <>
                <p className="mb-3 text-[12.5px] leading-relaxed text-gray">Is Agentic CRM steering you well? Your rating rolls into the daily improvement digest.</p>
